...
if [[ -n "$DASHSCOPE_API_KEY" ]]; then
  ssh_pw "sudo docker run -d --name $CONTAINER --network host -e SERVER_ID=$SERVER_ID -e SERVER_NAME=\\\"$SERVER_NAME\\\" -e MAX_QUOTA=$MAX_QUOTA -e HEARTBEAT_INTERVAL=$HEARTBEAT_INTERVAL -e BACKEND_WS_URL=$BACKEND_WS_URL -e DASHSCOPE_API_KEY=$DASHSCOPE_API_KEY -e LLM_BASE_URL=$LLM_BASE_URL -e LLM_MODEL=$LLM_MODEL -e LLM_MAX_TOKENS=$LLM_MAX_TOKENS -e LLM_ENABLE_THINKING=$LLM_ENABLE_THINKING -e LLM_TIMEOUT=$LLM_TIMEOUT -e EVENT_PERSIST_DISABLED=1 $IMAGE && sleep 3 && sudo docker ps --filter name=$CONTAINER && echo '--- logs ---' && sudo docker logs $CONTAINER --tail 15"
else
  ssh_pw "if test -f $ENV_FILE; then sudo docker run -d --name $CONTAINER --network host --env-file $ENV_FILE -e BACKEND_WS_URL=$BACKEND_WS_URL $IMAGE; else echo 'ERROR: no env-file (no prior container) and no DASHSCOPE_API_KEY in local env; set DASHSCOPE_API_KEY for first deploy'; exit 1; fi && sleep 3 && sudo docker ps --filter name=$CONTAINER && echo '--- logs ---' && sudo docker logs $CONTAINER --tail 15"
fi
