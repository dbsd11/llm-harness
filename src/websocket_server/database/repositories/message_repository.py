...
class MessageRepository(BaseRepository[Message]):
    ...
    def find_unacked_dispatch_by_task_id(self, task_id: str) -> Optional[Message]:
        """Find the unacked dispatch row for a given task_id (indexed lookup).

        Replaces the find_unacked_dispatch(limit=100) + python-side scan used in
        the WS ACK path: one indexed query instead of scanning up to 100 rows per
        ACK frame. Returns the oldest unacked dispatch for task_id, or None.
        """
        cm = get_connection_manager()
        with cm.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"SELECT * FROM {self.table_name} "
                f"WHERE message_type='dispatch' AND task_id=? "
                f"AND (acked IS NULL OR acked=0) "
                f"ORDER BY id ASC LIMIT 1",
                (task_id,),
            )
            row = cursor.fetchone()
            return Message.from_dict({k: row[k] for k in row.keys()}) if row else None
