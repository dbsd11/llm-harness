...
class CentralDispatcher:
    ...
    def _run(self) -> None:
        ...
        while not self._stop.is_set():
            try:
                # 拉取上限放大到 DISPATCH_POLL_LIMIT（默认 200）：原先 limit=10 + 跳过
                # in-flight 行的窗口策略，会让少数卡住/在途行永久占据最旧窗口，
                # 导致新分发永远不被看见（dispatcher 假死）。配合 forward_task 超时
                # 回收，卡住行最终会清理，窗口不再被长期占用。
                pending = msg_repo.find_unacked_dispatch(
                    limit=int(os.getenv("DISPATCH_POLL_LIMIT", "200"))
                )
            except Exception as e:
