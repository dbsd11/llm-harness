...
def build_message_history(scenario_id: str, trace_id: str = "", scenario: Any = None) -> List[Dict[str, Any]]:
    """
    Unified message timeline from persisted messages + events + reviews.
    ...
    if trace_id:
        ...
        for e in event_repo.find_by_trace_id(trace_id, limit=200):
            ...
            event_types_seen.add(e.event_type)
            rows.append({
    ...
    # Add human review records from tasks table
    task_repo = TaskRepository()
    tasks = task_repo.find_by_scenario_id(scenario_id)
    for t in tasks:
        if t.review_feedback or t.state == "pending_review":
            # Determine decision
            if t.state == "pending_review":
                decision = "待审核"
            ...
            rows.append({
                "time": str(t.completed_at or t.updated_at),
                "type": "人工验收",
                "kind": "review",
                "sender": "人工审核",
                "receiver": "系统",
                "content": f"{decision}：{t.review_feedback or '无反馈'}",
                "detail": {
                    "task_id": t.task_id,
                    "goal": t.goal,
                    "decision": decision,
                    "decision_color": decision_color,
                    "feedback": t.review_feedback,
                    "agent_role": t.agent_role,
                },
            })

    # Reconstruct missing terminal events if scenario is completed/failed
    if scenario and scenario.state in ("completed", "failed", "cancelled"):
        if scenario.completed_at:
            # Check if scenario.completed event exists
            if "scenario.completed" not in event_types_seen:
                rows.append({
                    "time": str(scenario.completed_at),
                    "type": "场景完成",
                    "kind": "event",
                    "sender": "系统",
                    "receiver": "—",
                    "content": f"场景状态：{scenario.state}",
                    "detail": {"event_type": "scenario.completed", "data": {"scenario_id": scenario_id}},
                })
