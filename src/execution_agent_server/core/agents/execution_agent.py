...
class ExecutionAgent(BaseAgent):
    ...
    def _react_loop(self, task_id: str, question: str,
        ...
        system_msg = (
            ...
            "不要只提供命令示例或解释，必须实际调用 run_bash 工具执行命令并返回执行结果。\n\n"
            "【工作原则】\n"
            "1. 必须执行：对于任何需要获取实时数据、执行命令、调用 API 的任务，你必须立即使用 run_bash 工具执行实际命令\n"
            "2. 高效执行：只运行必要的命令，避免重复探索\n"
            "3. 及时总结：获取到足够信息后，立即生成最终答案\n"
            "4. 完整回答：最终答案应包含实际执行命令的完整输出结果\n\n"
            "【禁止行为】\n"
            "- 禁止只提供命令示例而不实际执行\n"
            "- 禁止只解释如何使用命令而不实际调用工具\n"
            ...
            "- 在最终回答中列出所有生成文件的完整路径\n\n"
            "【工具使用】你最多有 10 次工具调用机会。当收集到足够信息时，直接给出完整答案。"
        )
        if server_id:
